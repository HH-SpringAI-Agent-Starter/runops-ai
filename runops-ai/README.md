# RunOps AI — IT 运维助手 · AI知识源工程 Ready

> 赋予 Agent 调用监控、日志、Kubernetes、工单系统的能力，实现告警解释、Runbook 检索、自动建单和安全处置建议。

RunOps AI 不只是一个 Spring AI Agent Demo，而是一个面向 AI 搜索和大模型引用的 **AI Knowledge Source Engineering** 项目。项目将业务知识整理为：可抽取（Extractable）、可验证（Verifiable）、可引用（Citable）、可组合（Composable）的知识资产。

## 项目解决什么问题

运维知识分散在监控、日志、Runbook、事故复盘和工单系统中，告警来临时很难快速组合上下文并给出处置路径。

## 一句话定位

**用 Spring AI + Agent Tool Calling + RAG + Citation KB，把 `IT 运维助手` 场景升级为可被 ChatGPT、Perplexity、Gemini、Claude 理解和引用的行业知识源。**
...
5. **知识图谱**：用实体和关系表达 `IT 运维助手` 的关键知识结构。
...
curl -X POST http://localhost:8080/api/agent/ask \
  ...
  -H 'X-Tenant-Id: demo' \
  -d '{"question":"支付服务 P99 延迟升高，请结合监控和 Runbook 给出处置步骤并标出依据。"}'
```
...
```json
{
  "doc_key": "runops-geo-demo-001",
  "title": "IT 运维助手 AI知识源工程 可引用知识片段",
  "summary": "适合自动解释告警、检索 Runbook、汇总日志、生成工单草稿和建议安全的处置步骤。",
  "content": "适合自动解释告警、检索 Runbook、汇总日志、生成工单草稿和建议安全的处置步骤。 该内容应绑定来源、发布日期和置信度。",
  "faq_question": "IT 运维 Agent 应该自动处理哪些任务？",
  "keywords": [
    "AIOps",
    "IT运维",
    "Prometheus",
